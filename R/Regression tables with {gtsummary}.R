...
#MY WORK+ATTEMPTS+NOTES

#QUESTION 1: Each of the univariate regression examples held the outcome (y =) constant, while varying the predictor variables with include =. You can also look at one predictor across several outcomes.
#Create a univariate regression table looking at the association between sex (sex_cat) as the
#x = variable and each of nsibs, sleep_wkdy, and sleep_wknd, and income.
...
tbl_uvregression(
	...
	y = sex_cat,
	include = c(
		starts_with('sleep'), nsibs, income
	),
	method = lm
)

#QUESTION 2: Fit a Poisson regression (family = poisson()) for the number of siblings, using at least 3 predictors of your choice. Create a nice table displaying your Poisson regression and its exponentiated coefficients.

#IMPORTANT TIP: do this in two parts: start with the logistic regression then fit the model

#part 1
poisson_model <- glm(nsibs ~ income + sex_cat + sleep_wkdy,
											data = nlsy, family = poisson())
#ERRORS:
#1. nsibs = outcome variable, other 3 are the predictors
#2: HAVE to use the glm model, not lm

#part 2: actually fit the model.
	...
	sex_cat ~ "Sex",
	income ~ "Income",
  sleep_wkdy ~ "Sleep on the Weekends"
		)
	)
	#ERRORS:
	#1. title

#QUESTION 5: Instead of odds ratios for wearing glasses, as in the example in the slides., we want risk ratios.
# We can do this by specifying in the regression family = binomial(link = "log").
# Regress glasses on eyesight_cat and sex_cat and create a table showing the risk ratios and confidence intervals from this regression.
